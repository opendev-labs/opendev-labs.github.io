        ...
        <form onSubmit={handleSubmit} className="w-full max-w-3xl mx-auto relative">
            <div className="bg-[#212121] border border-white/10 rounded-2xl px-4 py-2 flex flex-col justify-between transition-shadow focus-within:ring-1 focus-within:ring-white/20">
                <textarea
                    ...
                    placeholder="Deploy a Next.js mesh with SyncStack edge tunnel..."
                    className="w-full bg-transparent text-gray-200 text-base placeholder-gray-500 resize-none focus:outline-none py-2"
                    rows={1}
                    disabled={disabled}
                ...
                <div className="flex items-center justify-between mt-2">
                    ...
                    <button
                        ...
                        className={`h-8 w-8 rounded-full flex items-center justify-center transition-all disabled:cursor-not-allowed ${hasContent ? 'bg-white hover:bg-gray-200' : 'bg-neutral-800'}`}
                        aria-label="Send message"
                    >
                        <ArrowUpIcon className={`h-5 w-5 transition-colors ${hasContent ? 'text-black' : 'text-neutral-500'}`} />
                    </button>
                </div>
            </div>
            <ApiKeyModal
